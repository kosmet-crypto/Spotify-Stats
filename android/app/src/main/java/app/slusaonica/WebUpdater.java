 ...
 * at the same in-app URL as the bundled copy, so IndexedDB (the user's data) is unchanged.
...
final class WebUpdater {
     ...
     * {@code <meta name="app-native-api">} needs a newer APK and is not installed.
     */
    ...
    private static final Pattern NATIVE_API_META =
            Pattern.compile("<meta\\s+name=\"app-native-api\"\\s+content=\"(\\d+)\"");
    ...
    private static boolean looksValid(String html) {
        if (html.length() < 20000 || !html.contains("</html>") || !html.contains("id=\"app-root\"")) return false;
